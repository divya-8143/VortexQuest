...
export class Canvas2DRenderer {
  ...
  private renderWorldAndGameplay(gsm: GameStateManager): void {
    ...
    // 3. Ground Details (Grass Tufts, Pebbles, Dirt Patches)
    ctx.fillStyle = 'rgba(255, 255, 255, 0.04)';
    const tileSize = 64;
    for (let x = 0; x < currentZone.width; x += tileSize) {
      for (let y = 0; y < currentZone.height; y += tileSize) {
        if ((x + y) % 128 === 0) {
          // Pixelated Grass Tuft
          ctx.fillStyle = '#1e4620';
          ctx.fillRect(x + 10, y + 15, 3, 8);
          ctx.fillRect(x + 15, y + 10, 3, 13);
          ctx.fillRect(x + 20, y + 18, 3, 6);
        } else if ((x * y) % 192 === 0) {
          // Pebble
          ctx.fillStyle = '#3a4454';
          ctx.fillRect(x + 30, y + 40, 6, 4);
        }
      }
    }
    ...
    // 2. Render Scenery & Buildings with Soft Ground Shadows
    currentZone.decorations.forEach(deco => {
      switch (deco.type) {
        ...
        case 'HOUSE':
          ...
          ctx.fillRect(deco.x - 8, deco.y + hh - 8, hw + 16, 22);

          // Foundation & Timber Walls
        ...
        case 'TREE':
          // Soft Tree Shadow
          ctx.fillStyle = 'rgba(0,0,0,0.45)';
          ctx.beginPath(); ctx.ellipse(deco.x, deco.y + 12, (deco.w || 50) * 0.9, 14, 0, 0, Math.PI * 2); ctx.fill();
          // Trunk
          ...
          ctx.fillStyle = '#2d823a';
          ctx.beginPath(); ctx.arc(deco.x + 8, deco.y - 55, (deco.w || 50) * 0.45, 0, Math.PI * 2); ctx.fill();
          break;
    ...
    // 3. Swirling Active Portal Effects
    if (currentZone.portal) {
      ...
      portalGlow.addColorStop(0.5, 'rgba(163, 113, 247, 0.6)');
      portalGlow.addColorStop(1, 'rgba(0, 0, 0, 0)');
      ctx.fillStyle = portalGlow;
      ctx.beginPath(); ctx.arc(p.x, p.y, 50 + pulse, 0, Math.PI * 2); ctx.fill();

      // Swirling Orbiting Particle Embers
      for (let i = 0; i < 4; i++) {
        const angle = this.animTimer * 3 + (i * Math.PI / 2);
        const px = p.x + Math.cos(angle) * 32;
        const py = p.y + Math.sin(angle) * 32;
        ctx.fillStyle = '#ffd700';
    ...
    // 4. Interactive NPCs with Soft Shadows
    ...
    // 1. Monsters Overhaul (Bouncing Slimes & Animated Stance)
    currentZone.enemies.forEach(enemy => {
      if (enemy.hp > 0) {
        // Soft Enemy Ground Shadow
        ctx.fillStyle = 'rgba(0,0,0,0.5)';
        ctx.beginPath(); ctx.ellipse(enemy.x, enemy.y + enemy.radius * 0.7, enemy.radius * 1.1, enemy.radius * 0.45, 0, 0, Math.PI * 2); ctx.fill();
        ...
        if (enemy.isBoss) {
          ...
          // Bouncing Slime Animation
          const bounce = Math.abs(Math.sin(this.animTimer * 5)) * 6;
          ctx.fillStyle = '#3fb950';
          ctx.beginPath();
          ctx.ellipse(enemy.x, enemy.y - bounce, enemy.radius + bounce * 0.3, enemy.radius - bounce * 0.3, 0, 0, Math.PI * 2);
          ctx.fill();
          ctx.strokeStyle = '#1e4620'; ctx.lineWidth = 2; ctx.stroke();

          // Slime Eyes
          ctx.fillStyle = '#ffffff';
          ctx.beginPath(); ctx.arc(enemy.x - 6, enemy.y - 4 - bounce, 3, 0, Math.PI * 2); ctx.fill();
          ctx.beginPath(); ctx.arc(enemy.x + 6, enemy.y - 4 - bounce, 3, 0, Math.PI * 2); ctx.fill();
          ctx.fillStyle = '#000000';
          ctx.beginPath(); ctx.arc(enemy.x - 5, enemy.y - 4 - bounce, 1.5, 0, Math.PI * 2); ctx.fill();
          ctx.beginPath(); ctx.arc(enemy.x + 7, enemy.y - 4 - bounce, 1.5, 0, Math.PI * 2); ctx.fill();
        } else {
          ctx.fillStyle = enemy.type === 'MAGE' ? '#a371f7' : '#f85149';
    ...
    // 1. Hero Knight Sprite with Directional Stance
  ...
  private renderHUD(gsm: GameStateManager): void {
    ...
    // 2. HUD Cards & Stat Icons (❤️, 🔷, ⭐)
    ctx.fillStyle = 'rgba(15, 20, 30, 0.92)';
    ctx.strokeStyle = '#c9a050'; ctx.lineWidth = 1.5;
    ctx.fillRect(15, 15, 260, 115);
    ctx.strokeRect(15, 15, 260, 115);
    ...
    // HP Bar with ❤️ Heart Icon
    ctx.fillStyle = '#f85149'; ctx.font = '13px system-ui';
    ctx.fillText('❤️', 28, 59);
    const hpRatio = Math.max(0, player.stats.hp / player.stats.maxHp);
    ctx.fillStyle = '#161e2b'; ctx.fillRect(48, 48, 175, 13);
    ctx.fillStyle = '#f85149'; ctx.fillRect(48, 48, 175 * hpRatio, 13);
    ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 1; ctx.strokeRect(48, 48, 175, 13);
    ctx.fillStyle = '#ffffff'; ctx.font = '11px Rajdhani, sans-serif';
    ctx.fillText(`HP: ${Math.ceil(player.stats.hp)} / ${player.stats.maxHp}`, 55, 58);

    // Mana Bar with 🔷 Crystal Icon
    ctx.fillStyle = '#58a6ff'; ctx.font = '13px system-ui';
    ctx.fillText('🔷', 28, 79);
    const manaRatio = Math.max(0, player.stats.mana / player.stats.maxMana);
    ctx.fillStyle = '#161e2b'; ctx.fillRect(48, 68, 175, 13);
    ctx.fillStyle = '#1f6beb'; ctx.fillRect(48, 68, 175 * manaRatio, 13);
    ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 1; ctx.strokeRect(48, 68, 175, 13);
    ctx.fillStyle = '#ffffff'; ctx.font = '11px Rajdhani, sans-serif';
    ctx.fillText(`MP: ${Math.ceil(player.stats.mana)} / ${player.stats.maxMana}`, 55, 78);

    // XP Bar with ⭐ Star Icon
    ctx.fillStyle = '#ffd700'; ctx.font = '13px system-ui';
    ctx.fillText('⭐', 28, 97);
    const xpRatio = Math.max(0, player.stats.xp / player.stats.maxXp);
    ctx.fillStyle = '#161e2b'; ctx.fillRect(48, 88, 175, 11);
    ctx.fillStyle = '#3fb950'; ctx.fillRect(48, 88, 175 * xpRatio, 11);
    ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 1; ctx.strokeRect(48, 88, 175, 11);
    ctx.fillStyle = '#ffffff'; ctx.font = '10px Rajdhani, sans-serif';
    ctx.fillText(`XP: ${player.stats.xp} / ${player.stats.maxXp}`, 55, 96);
    ...
    // 4. MINIMAP RADAR WITH GOLD TRIM & "N" COMPASS INDICATOR
    ...
    // Golden "N" Compass Marker
    ctx.fillStyle = '#ffd700'; ctx.font = 'bold 12px Cinzel, serif'; ctx.textAlign = 'center';
    ctx.fillText('N', mapX, mapY - mapR + 12);
